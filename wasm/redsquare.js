...
function findWasmBinary() {
  return locateFile('redsquare.wasm');
}
...
// Begin JS library code
  ...
  function _em_getch(timeout_ms) {
      return Asyncify.handleAsync(function() {
        return new Promise(function(resolve) {
          // If we have buffered input, return immediately
          if (Module._inputBuffer && Module._inputBuffer.length > 0) {
          ...
          function onData(data) {
            ...
            // Parse escape sequences for arrow keys, function keys
            var key = parseKey(bytes);
            if (key !== null) {
              resolve(key);
            } else {
              // Return first byte, buffer rest
              if (!Module._inputBuffer) Module._inputBuffer = [];
              for (var i = 1; i < bytes.length; i++) {
                Module._inputBuffer.push(bytes[i]);
              }
              resolve(bytes[0]);
            }
          }
          ...
          function parseKey(buf) {
            if (buf.length === 1) return null; // single char, handle normally
            ...
            if (buf[0] === 0x1b && buf.length >= 3 && buf[1] === 0x5b) {
              ...
              // Function keys: ESC [ 1 1 ~ = F1, ESC [ 1 2 ~ = F2, etc
              if (buf.length >= 5 && buf[2] === 0x31) {
                if (buf[3] === 0x31 && buf[4] === 0x7e) return 0x109 + 1; // F1
                if (buf[3] === 0x32 && buf[4] === 0x7e) return 0x109 + 2; // F2
                if (buf[3] === 0x33 && buf[4] === 0x7e) return 0x109 + 3; // F3
                if (buf[3] === 0x34 && buf[4] === 0x7e) return 0x109 + 4; // F4
              }
              // ESC [ O P/Q/R/S for F1-F4 (alternate)
              if (buf.length >= 3 && buf[1] === 0x4f) {
                switch(buf[2]) {
            ...
            // ESC O sequences (xterm style function keys)
          ...
          // Handle timeout (for halfdelay / nodelay)
          if (timeout_ms > 0) {
  ...
  instrumentWasmImports(imports) {
        var importPattern = /^(em_getch|invoke_.*|__asyncjs__.*)$/;
